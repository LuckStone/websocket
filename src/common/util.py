...
def Parse2Int(value, default=None):
    try:
        return int(value)
    except:
        return default
...
def PageCondition(**args):
    ...
    return page_number, (page_index-1) * page_number, param if len(param)==1 else param.items()
...
class TreeResult(LawResult):
    ...
    def __str__(self):
        return "TreeResult<'total':%d, 'return':%s>"%(len(self.data), str(self.data))
...
class Result(LawResult):
    def __init__(self,data, result=0, msg="done", httpcode=500):
    ...
    def to_json(self):
        return {'error_code':self.result,'error_msg':self.message,'content':self.content}
...
def AESEncrypt(msg,sec_dir='frame/conf/AES'):
    ...
    return crypter.Encrypt(str(msg))
...
def AESDecrypt(text,sec_dir='frame/conf/AES'):
    ...
    crypter = keyczar.Crypter.Read(sec_dir)
    return crypter.Decrypt(str(text))
